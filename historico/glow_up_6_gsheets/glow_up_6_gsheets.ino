...
const unsigned long intervaloGoogleSheets = 60000; // 60 segundos
...
void enviarDadosGoogleSheets() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[Google Sheets] Wi-Fi desconectado. Tentando reconectar...");
    WiFi.reconnect();
    return;
  }
  ...
  Serial.println("\n[Google Sheets] Iniciando envio...");

  if (!http.begin(client, GOOGLE_SCRIPT_URL)) {
    Serial.println("[Google Sheets] Falha no http.begin()");
    return;
  }
  ...
  Serial.println("[Google Sheets] Payload enviado:");
  Serial.println(payload);

  int httpCode = http.POST(payload);

  Serial.print("[Google Sheets] Codigo HTTP: ");
  Serial.println(httpCode);

  if (httpCode > 0) {
    String resposta = http.getString();
    Serial.println("[Google Sheets] Resposta:");
    Serial.println(resposta);
  } else {
    Serial.print("[Google Sheets] Erro no POST: ");
...
void loop() {
  ...
  // -----------------------------------------------------------------
  // BLOCO 3: ALARMES SONOROS RÍTMICOS
  // -----------------------------------------------------------------
  ...
  // -----------------------------------------------------------------
  // BLOCO 4: SURTOS DE ANIMAÇÃO DINÂMICA
  // -----------------------------------------------------------------
  ...
  // -----------------------------------------------------------------
  // BLOCO 5: ENVIO AO GOOGLE SHEETS
  // -----------------------------------------------------------------
